...
\begin{table}[htbp]
\centering
\caption{Métriques de télémétrie — Next.js dev (Turbopack), port 3000}
...
& & (ms) & (ms) & (ms) & (ms) & \\
\midrule
Next.js Turbopack & 3000 & 100{,}89 & 29{,}11 & 130{,}48 & \textbf{+29{,}58} & \textbf{6{,}26} \\
\midrule
\multicolumn{7}{l}{\small $N_{\mathrm{calib}} = 50$,
  $N_{\mathrm{load}} = 38$ (après filtre IQR).
  Cohen~$d = 0{,}48$ (effet moyen). P99~$= 363$~ms.}\\
\bottomrule
\end{tabular}
\end{table}
...
Le Tableau~\ref{tab:stress} présente la courbe de saturation du serveur
Next.js sous charge concurrente croissante. Trois régimes distincts sont
observés.

\begin{table}[htbp]
\centering
\caption{Saturation du serveur Next.js dev sous charge concurrente}
\label{tab:stress}
\begin{tabular}{rrrrl}
\toprule
$C$ & $\bar{T}_{\mathrm{load}}$ (ms) & P99 (ms) & Succès (\%) & Régime \\
\midrule
5  & 5\,274 & 5\,451 & 100 & Compilation Turbopack (à froid) \\
10 & 965   & 1\,256 & 100 & Cache chaud — event loop stable \\
20 & 1\,000 & 1\,652 & 100 & Plateau de saturation \\
\textbf{50} & \textbf{2\,007} & \textbf{3\,965} & \textbf{54} & \textbf{Effondrement — 46\% d'échecs} \\
\bottomrule
\end{tabular}
\end{table}
