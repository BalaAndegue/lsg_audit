...
\centering
\begin{tikzpicture}[
    machine/.style={draw, rounded corners=4pt, minimum width=2.5cm,
  ...
  % Machine B
  \node[machine, fill=orange!6] (B) at (6,0)
    {\textbf{Machine B}\\[2pt]\scriptsize Serveur dev\\Next.js 14};

  % WiFi link
  \draw[arr, double, double distance=1.5pt, gray!60] (A.east) --
    node[above, lbl]{Wi-Fi 802.11 \ ($\bar{T}=195$~ms)}
    node[below, lbl]{requêtes HTTP / métriques TTFB}
    (B.west);

  % Binding modes on B
  \node[warn, below=0.4cm of B, xshift=-0.2cm]
    (bad) {\texttt{0.0.0.0:3000} \textbf{(LSG)}};
  \node[good, below=0.9cm of B, xshift=-0.2cm]
    {\texttt{127.0.0.1:3000} \checkmark};

  % Red arrow showing LAN access
  \draw[arr, red!70, bend right=20, dashed]
    (A.south east) to
    node[below, lbl, red!70, pos=0.55]{accessible depuis le LAN}
    (bad.west);

  % Lock icon for good config
  \node[lbl, gray] at ($(B.south)+(0,-1.55)$) {\textit{(config.\ sécurisée)}};
\end{tikzpicture}
\caption{Topologie du banc d'essai. Un service lié à \texttt{0.0.0.0}
est accessible depuis toutes les interfaces du LAN (\textit{Localhost
Security Gap}), contrairement à \texttt{127.0.0.1}.}
\label{fig:topology}
\end{figure}
...
\begin{tikzpicture}[
    >=Stealth,
    evt/.style={circle, fill=black, inner sep=1.8pt},
    lbl/.style={font=\scriptsize},
    brace/.style={decorate,
                  decoration={brace, amplitude=5pt, raise=2pt}}
]
  % Time axis
  \draw[->, thick] (-0.4,0) -- (8.8,0) node[right, lbl]{temps};

  % Key events
  \coordinate (t0)  at (0.0, 0);
  \coordinate (tc)  at (2.8, 0);
  \coordinate (ts)  at (5.6, 0);
  \coordinate (tend)at (8.0, 0);
  ...
  % Labels below axis
  \node[lbl, below=0.18cm] at (t0)  {$t_0$\\SYN};
  \node[lbl, below=0.18cm] at (tc)  {$t_\mathrm{connect}$\\TCP~ACK};
  \node[lbl, below=0.18cm] at (ts)  {$t_\mathrm{TTFB}$\\1\textsuperscript{er}~octet};

  % Braces above axis
  \draw[brace, blue!65]  (t0)  -- (tc)
    node[midway, above=8pt, lbl, blue!65]
    {$T_\mathrm{r\acute{e}seau}$};
  \draw[brace, red!70]   (tc)  -- (ts)
    node[midway, above=8pt, lbl, red!70]
    {$\hat{T}_\mathrm{calcul}$ \emph{(isolé)}};
  \draw[brace, gray!55]  (ts)  -- (tend)
    node[midway, above=8pt, lbl, gray!60]
    {transfert \emph{(avorté)}};

  % libcurl annotation arrows
  \draw[->, dashed, blue!50, thin]
    (1.4, -1.0) node[lbl, blue!60, left=1pt]
      {\texttt{CURLINFO\_CONNECT\_TIME}}
    -- (1.4, -0.08);
  \draw[->, dashed, red!50, thin]
    (4.2, -1.0) node[lbl, red!60, right=1pt]
      {\texttt{CURLINFO\_STARTTRANSFER\_TIME}}
    -- (4.2, -0.08);
\end{tikzpicture}
\caption{Décomposition temporelle du RTT via les compteurs internes de
\texttt{libcurl}. Le callback d'écriture avorte le transfert du corps
dès le premier octet~; $\hat{T}_\mathrm{calcul}$ est capturé avant
cet abandon.}
\label{fig:rtt}
\end{figure}
...
\begin{axis}[
    ...
    y label style={color=blue!70!black},
    ymin=0, ymax=7200,
    axis y line*=left,
    axis x line*=bottom,
...
\addplot[fill=blue!25, draw=blue!60!black, thick] coordinates {
    (1,5274)(2,965)(3,1000)(4,2007)
};
% Saturation zone
\fill[red!8] (axis cs:3.5,0) rectangle (axis cs:4.5,7200);
\draw[red!50, dashed, thin] (axis cs:3.5,0) -- (axis cs:3.5,7200)
    node[pos=0.92, right, font=\scriptsize, red!70]{seuil};
\end{axis}

% Right axis — success rate line
\begin{axis}[
    at={(ax1.south east)}, anchor=south east,
    width=0.92\columnwidth, height=5.2cm,
    enlarge x limits=0.25,
    xtick={1,2,3,4}, xticklabels={},
    ylabel={Taux de succès (\%)},
    y label style={color=red!70!black},
    axis y line*=right,
    axis x line*=none,
    ymin=0, ymax=115,
    tick label style={font=\scriptsize},
...
\addplot[red!70!black, thick, mark=*, mark size=2.8pt, dashed] coordinates {
    (1,100)(2,100)(3,100)(4,54)
};
\end{axis}
\end{tikzpicture}
\caption{Saturation du serveur Next.js dev. Barres bleues~:
latence moyenne. Ligne rouge pointillée~: taux de succès.
La zone ombrée indique l'effondrement à $C=50$ (54\% de succès,
46\% d'échecs).}
